...
| `fieldConfiguration.options[].schemeSubTreeInclusions`      | string  | n                             |         | If only selected subtrees of a hierarchical concept should be displayed, the IDs of the subtrees' roots can be defined here. Only considered in filters for a hierarchical concept. No effect on cards and detail pages.                                                                                                      |
...
| `search.filters.expandRootItems`                 | boolean          | n         | false                                                    | Expand root categories of hierarchical concept per default (on first load) true / false                                                                                                                                                                                      |
...
| `footer.links[].reloadDocument` | boolean          | n         | Default `true`. If `true`, use document navigation instead of client side routing when the link is clicked: the browser will handle the transition normally                                           |
